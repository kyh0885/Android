...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bottomNavigation.setOnNavigationItemSelectedListener(new BottomNavigationView.OnNavigationItemSelectedListener() {
            @Override
            public boolean onNavigationItemSelected(@NonNull MenuItem item) { //탭을 선택하면 이 것들이 실행됨
    ...
    class MainHandler extends Handler {
        ...
        public void handleMessage(@NonNull Message msg) {
            ...
            if(bottomNavigationIndex ==1)
                fragment2.recvDataProcess(data);
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == REQUEST_CODE_LOGIN_ACTIVITY) {
            if(resultCode == RESULT_OK) {
                ...
                clientThread.start();//버튼을 비활성화 시키는 코드 추가 왜냐면 tab3에서 실행될때 tab1에서의 로그인이 되지 않도록
                if(bottomNavigationIndex == 2)
                    fragment3Telnet.toggleButtonStart.setEnabled(false);
            } else if(resultCode == RESULT_CANCELED) {
                if(clientThread != null) { //Null이기 때문에 stop 안되고 그냥 종료됨
